...
function uniquifyArray(words) {
    // TODO:
    const newArray = [];
    words.forEach((word) => !newArray.includes(word) && newArray.push(word));
    return newArray;
}
...
function sum(array) {
    ...
    array.forEach((item) => {
        if (typeof item === "string") {
            numbersArray.push(item.length);
        } else if (typeof item === "boolean") {
            numbersArray.push(item === true ? 1 : 0);
        } else {
            numbersArray.push(item);
        }
    });
    return numbersArray.reduce((total, num) => total + num);
}
...
function greatestProduct(matrix) {
    ...
    for (
        ...
        indexHorizontal + 3 < matrix[0].length;
        indexHorizontal++
    ) {
        for (
            let indexVertical = 0;
            indexVertical + 3 < matrix.length;
            indexVertical++
        ) {
            const product =
                matrix[indexVertical][indexHorizontal] *
                matrix[indexVertical + 1][indexHorizontal] *
                matrix[indexVertical + 2][indexHorizontal] *
                matrix[indexVertical + 3][indexHorizontal];
            if (product > result) {
                result = product;
            }
        }
